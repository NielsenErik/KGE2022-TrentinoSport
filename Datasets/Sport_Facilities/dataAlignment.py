...
def pagineGialle():
    ...
    for k in range(len(pagineGialle)):
        address.append(pagineGialle[k]['Address']['Address'][0])
        city.append(pagineGialle[k]['Address']['Address'][1])
    for k in range(len(pagineGialle)):
        openings.append(pagineGialle[k]['Openings'])
    pgColoums = ['Name', 'Address', 'Municipality','OpeningHours', 'Telephone', 'Type']
    d = {pgColoums[0] : names, 
        pgColoums[1] : address, 
        pgColoums[2] : city, 
        pgColoums[3] : openings, 
        pgColoums[4] : tel, 
        pgColoums[5] : typeFac
        }
...
def trentoFac():
    ...
    for k in range(len(tn)):
        ...
        address.append(tn[k]['infos']['Indirizzo'])
        city.append("Trento")
